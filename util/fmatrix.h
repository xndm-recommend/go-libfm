...
// Implementation
template<typename T>
void LargeSparseMatrix<T>::saveToBinaryFile(std::string filename) {
...
template<typename T>
void LargeSparseMatrix<T>::saveToTextFile(std::string filename) {
    std::cout << "printing to " << filename << std::endl;
    std::cout.flush();
...
template<typename T>
void LargeSparseMatrixHD<T>::readcache() {
...
template<typename T>
LargeSparseMatrixHD<T>::LargeSparseMatrixHD(std::string filename, uint64 cache_size) {
    ...
    std::cout << "num entries in cache=" << num_entries_in_cache << "\tnum rows in cache=" << num_rows_in_cache
              << std::endl;
...
template<typename T>
uint LargeSparseMatrixHD<T>::getNumRows() {
    return num_rows;
};

template<typename T>
uint LargeSparseMatrixHD<T>::getNumCols() {
    return num_cols;
};

template<typename T>
uint64 LargeSparseMatrixHD<T>::getNumValues() {
    return num_values;
};

template<typename T>
void LargeSparseMatrixHD<T>::next() {
    row_index++;
...
template<typename T>
void LargeSparseMatrixHD<T>::begin() {
...
template<typename T>
bool LargeSparseMatrixHD<T>::end() {
    return row_index >= num_rows;
}

template<typename T>
sparse_row<T> &LargeSparseMatrixHD<T>::getRow() {
    return data(position_in_data_cache);
}

template<typename T>
uint LargeSparseMatrixHD<T>::getRowIndex() {
    return row_index;
}

template<typename T>
void LargeSparseMatrixMemory<T>::begin() {
    index = 0;
}

template<typename T>
bool LargeSparseMatrixMemory<T>::end() {
    return index >= data.dim;
}

template<typename T>
void LargeSparseMatrixMemory<T>::next() {
    index++;
}

template<typename T>
sparse_row<T> &LargeSparseMatrixMemory<T>::getRow() {
    return data(index);
}

template<typename T>
uint LargeSparseMatrixMemory<T>::getRowIndex() {
    return index;
}

template<typename T>
uint LargeSparseMatrixMemory<T>::getNumRows() {
    return data.dim;
}

template<typename T>
uint LargeSparseMatrixMemory<T>::getNumCols() {
    return num_cols;
}

template<typename T>
uint64 LargeSparseMatrixMemory<T>::getNumValues() {
    return num_values;
}
