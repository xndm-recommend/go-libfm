...
// Implementation
template<typename T>
T DMatrix<T>::get(uint x, uint y) {
//assert((x < dim1) && (y < dim2));
    return value[x][y];
}

template<typename T>
DMatrix<T>::DMatrix(uint p_dim1, uint p_dim2) {
    dim1 = 0;
    dim2 = 0;
    value = NULL;
...
template<typename T>
void DMatrix<T>::assign(DMatrix<T> &v) {
...
template<typename T>
void DMatrix<T>::setSize(uint p_dim1, uint p_dim2) {
...
template<typename T>
T &DMatrix<T>::operator()(unsigned x, unsigned y) {
//  assert((x < dim1) && (y < dim2));
    return value[x][y];
}

template<typename T>
T DMatrix<T>::operator()(unsigned x, unsigned y) const {
//  assert((x < dim1) && (y < dim2));
    return value[x][y];
}

template<typename T>
T *DMatrix<T>::operator()(unsigned x) const {
//  assert((x < dim1));
    return value[x];
}

template<typename T>
void DMatrix<T>::save(std::string filename, bool has_header) {
...
template<typename T>
void DMatrix<T>::saveToBinaryFile(std::string filename) {
...
template<typename T>
void DMatrix<T>::loadFromBinaryFile(std::string filename) {
...
template<typename T>
void DMatrix<T>::load(std::string filename) {
...
template<typename T>
T DVector<T>::get(uint x) {
    return value[x];
}

template<typename T>
void DVector<T>::setSize(uint p_dim) {
    if (p_dim == dim) { return; }
    if (value != NULL) {
        MemoryLog::getInstance().logFree("dvector", sizeof(T), dim);
        delete[] value;
...
template<typename T>
T &DVector<T>::operator()(unsigned x) {
    return value[x];
}

template<typename T>
T DVector<T>::operator()(unsigned x) const {
    return value[x];
}

template<typename T>
void DVector<T>::init(T v) {
...
template<typename T>
void DVector<T>::assign(DVector<T> &v) {
    if (v.dim != dim) { setSize(v.dim); }
    for (uint i = 0; i < dim; i++) {
        value[i] = v.value[i];
    }
...
template<typename T>
void DVector<T>::save(std::string filename) {
    std::ofstream out_file(filename.c_str());
    if (out_file.is_open()) {
        for (uint i = 0; i < dim; i++) {
...
template<typename T>
void DVector<T>::saveToBinaryFile(std::string filename) {
    std::ofstream out(filename.c_str(), std::ios_base::out | std::ios_base::binary);
    if (out.is_open()) {
...
template<typename T>
void DVector<T>::load(std::string filename) {
    std::ifstream in_file(filename.c_str());
    if (!in_file.is_open()) {
        throw "Unable to open file " + filename;
    }
...
template<typename T>
void DVector<T>::loadFromBinaryFile(std::string filename) {
    std::ifstream in(filename.c_str(), std::ios_base::in | std::ios_base::binary);
    if (in.is_open()) {
        uint file_version;
        uint data_size;
